import {_object, Vector2, AudioPlayer, BoxCollider, random, Label, BaseButton, Texture, ParallaxBackground, ParallaxLayer} from "./Import.js";
import { currentScene, collisions } from "./Variables.js"; 
export class GameObject extends _object
{
    #rotation;
    constructor(name = "undefined", position = new Vector2(), size = new Vector2())
    {
    ...
    add_child(obj) 
    {
       ...
       if ((obj instanceof BoxCollider) && this.collider === null) // Si el objeto es una colision, se añade como colision
        ...
        if (obj instanceof Label && !(obj instanceof Text))
        {
            console.error(`El objeto ${obj} no se puede agregar como hijo \n`);
            console.warn("Para añadir un objeto de texto debes usar la clase Text envés de Label.");
            return; 
        }
        if (obj instanceof BaseButton || obj instanceof Texture)
        {
            console.error(`El objeto ${obj.name} no se puede agregar como hijo \n`);
            console.warn("Los objetos de Interfaz no se pueden añadir como hijos de "+ this.name);
            return; 
        }

        if (obj instanceof ParallaxBackground || obj instanceof ParallaxLayer)
        {
            console.error(`El objeto ${obj.name} no se puede agregar como hijo de ${this.name} \n`);
            return; 
        }
    ...
    _processChildren() 
    {
        for (let obj of this.child) 
        {
            ...
            if (this.#isFunction(obj.move_and_slide)) obj.move_and_slide();
            if (this.#isFunction(obj._process)) obj._process();
            if (this.#isFunction(obj.process)) obj.process();
        }
    }
    addCollision()
    {
        if (this.child.length > 0)
        {
            this.child.forEach(obj => {
                ...
                if (obj.child !== undefined && obj.addCollision !== undefined) obj.addCollision();
            });
        }
    }
}