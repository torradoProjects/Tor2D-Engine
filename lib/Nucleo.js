...
import { canvas, ctx, bufferCanvas, bufferCtx, uiCanvas, uiCtx, debugs, currentScene, SceneTree} from "./Variables.js";
import { Label, Vector2, UI, SpatialGrid, Time, Camera, isVisible, AudioPlayer, BaseButton, Texture, ParallaxBackground, Input, Collision} from "./Import.js";
export class TOR2D // Nucleo
{
    ...
    constructor(config)
    {
        ...
        this.spatialGrid = new SpatialGrid(250); 
    }
    #Clear()
    {
    ...
    #Process(currentTime)
    {
        ...
        if (scene)
        {
            ...
            if (scene.child.length > 1)
            {
                let obj = [];
                scene.child.forEach(child => {
                    if (child.collider != undefined) obj.push(child.collider);
                });
                this.add_in_grid(obj);
            }
    ...
    #Loop(currentTime)
    {
        ...
        bufferCtx.imageSmoothingEnabled = this.#texture_filter; // apñica el filtro
