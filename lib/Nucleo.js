...
import { canvas, ctx, bufferCanvas, bufferCtx, uiCanvas, uiCtx, debugs, currentScene, SceneTree, collisions, spatialGrid} from "./Variables.js";
import { Label, Vector2, UI, SpatialGrid, Time, Camera, isVisible, AudioPlayer, BaseButton, Texture, ParallaxBackground, Input, add_in_grid, saveData, loadData, deleteData} from "./Import.js";
export class TOR2D // Nucleo
{
    ...
    #Loop(currentTime)
    {
        ...
        if (debugs) this.debugFPS.Render(); // mostramos los fps si el debugs es true
    ...
    change_scene(scene)
    {
        if (scene.name === currentScene.name) return;
        this.#clear_scene();
    ...
    reload_scene()
    {
        saveData("currentScene", currentScene.name);
        location.reload();
    }
    start(scene) // funcion para iniciar el loop y la escena como principal
	{
		window.addEventListener("load", ()=>{
            ...
            if (loadData("currentScene") !== null)
            {
                let name = loadData("currentScene");
                currentScene = SceneTree.find(scene => scene.name === name);
                deleteData("currentScene");
            } else currentScene = scene; // asigna la escena como escena en ejecucion

			if (typeof currentScene.ready === "function") currentScene.ready(); // ejecuta la funcion ready por unica vez
